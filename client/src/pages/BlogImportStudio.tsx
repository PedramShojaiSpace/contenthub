...
type RefinedArticle = {
  ...
  longFormMinimumWords: number;
  longFormTargetWords: string;
};
...
export default function BlogImportStudio() {
  ...
  const refinedWordCount = useMemo(() => refined?.articleMarkdown.trim() ? refined.articleMarkdown.trim().split(/\s+/).length : 0, [refined?.articleMarkdown]);
  const meetsLongFormMinimum = refined ? refinedWordCount >= refined.longFormMinimumWords : false;
  ...
  const handleSave = () => {
    if (!refined) return;
    if (!meetsLongFormMinimum) return toast.error(`Keep the article at or above ${refined.longFormMinimumWords.toLocaleString()} words before saving the review draft.`);
    saveDraft.mutate({
      title: refined.title,
  ...
  const handleWordPress = () => {
    ...
    if (!meetsLongFormMinimum) return toast.error(`Expand the article to at least ${refined.longFormMinimumWords.toLocaleString()} words before WordPress handoff.`);
  ...
  const handleSubstack = () => {
    ...
    if (!meetsLongFormMinimum) return toast.error(`Expand the article to at least ${refined.longFormMinimumWords.toLocaleString()} words before creating a Substack review draft.`);
  ...
  return (
    <main className="mx-auto w-full max-w-7xl space-y-7 px-5 py-8 text-slate-900">
      <header className="border-b border-slate-200 pb-6">
        ...
        <p className="mt-2 max-w-3xl text-slate-600">Import a complete article, vlog, or transcript as the starting idea. The Studio expands it into a serious long-form Urban Monk article, then lets you review its SEO, CTA, and category before creating a WordPress or Substack draft.</p>
      </header>
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <div className="space-y-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="flex justify-between text-xs text-slate-500"><span>{wordCount.toLocaleString()} source words</span><span>The source is expanded; it is not treated as final copy.</span></div>
          <button type="button" disabled={busy || !selectedCtaId} onClick={handleRefine} className="inline-flex w-full items-center justify-center gap-2 rounded-md bg-emerald-700 px-4 py-2.5 font-semibold text-white disabled:opacity-50">{refine.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}{refine.isPending ? "Developing long-form article…" : "Develop into long-form article + SEO"}</button>
        </div>
        ...
        <div className="space-y-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          {!refined ? <p className="rounded-md bg-slate-50 p-5 text-sm text-slate-600">The editable complete article appears here after refinement. Existing citations are retained, unsupported claims are flagged, and the CTA/category come only from your selected approved records.</p> : <>
            ...
            <div className={`rounded-md border p-3 text-sm ${meetsLongFormMinimum ? "border-emerald-200 bg-emerald-50 text-emerald-950" : "border-rose-200 bg-rose-50 text-rose-950"}`}><strong>Long-form depth check: {refinedWordCount.toLocaleString()} words</strong><p className="mt-1 text-xs">Target: {refined.longFormTargetWords} words before CTA. Minimum for any review draft, WordPress handoff, or Substack draft: {refined.longFormMinimumWords.toLocaleString()} words. {meetsLongFormMinimum ? "The current editable article meets the workflow minimum." : "Expand this editable article before it can move forward."}</p></div>
            ...
            <button type="button" disabled={busy || contentItemId !== null || !meetsLongFormMinimum} onClick={handleSave} className="rounded-md border border-slate-300 px-4 py-2 font-semibold disabled:opacity-50">{contentItemId ? "Internal review draft saved" : "Save review draft"}</button>
            <div className="grid gap-3 sm:grid-cols-2"><div className="rounded-md border border-slate-200 p-3"><strong className="text-sm">WordPress handoff</strong><label className="mt-2 flex items-center gap-2 text-sm"><input type="checkbox" checked={publishLive} onChange={e => setPublishLive(e.target.checked)} />I completed final review and intend to publish live</label><button type="button" disabled={busy || contentItemId === null || !imageCandidate || !selectedCategoryId || !meetsLongFormMinimum} onClick={handleWordPress} className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-md bg-slate-900 px-4 py-2.5 font-semibold text-white disabled:opacity-50"><Send className="h-4 w-4" />{publishLive ? "Verify draft, then publish live" : "Create and verify WordPress draft"}</button><p className="mt-2 text-xs text-slate-500">Every handoff creates and verifies a WordPress draft first. Live publication occurs only after all title, status, SEO, category, image, canonical, and long-form checks pass.</p></div><div className="rounded-md border border-indigo-200 bg-indigo-50 p-3"><strong className="text-sm text-indigo-950">Substack handoff</strong><p className="mt-2 text-xs leading-5 text-indigo-900">Creates a private Substack draft from this reviewed article. It does not publish, email subscribers, or auto-share.</p><button type="button" disabled={busy || contentItemId === null || !meetsLongFormMinimum} onClick={handleSubstack} className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-md border border-indigo-700 px-4 py-2.5 font-semibold text-indigo-800 disabled:opacity-50"><Send className="h-4 w-4" />Create Substack draft for review</button></div></div>

        ...
        <div className="space-y-2">
          ...
          {showVidiqPanel && (
            <div className="rounded-lg border border-border bg-muted/40 p-3 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">vidIQ{vidiqResearch.data ? ` — "${vidiqResearch.data.keyword}"` : " Keyword Data"}</span>
              ...
              {vidiqResearch.isFetching ? (
                ...
                <div className="rounded-md border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900 dark:border-amber-700 dark:bg-amber-950/30 dark:text-amber-200">
                  ...
                  <p className="mt-1 text-xs">No zero scores were recorded. Your session keyword remains unchanged.</p>
                  <Button type="button" variant="outline" size="sm" className="mt-3 h-8 bg-background text-xs" onClick={() => { void vidiqResearch.refetch(); }}>
                    <RotateCcw className="mr-1.5 h-3.5 w-3.5" /> Retry vidIQ research
                  </Button>
                </div>
              ) : vidiqResearch.data ? (
                <>
                  ...
                  {vidiqResearch.data.volume === 0 && (
                    <p className="text-xs text-muted-foreground">vidIQ reported no direct volume score for this exact phrase. Related terms below may offer adjacent topics.</p>
                  )}
                ...
                <p className="text-sm text-muted-foreground">Enter a keyword and choose Research to check YouTube opportunity.</p>
              )}
            </div>
          )}
        </div>

...
const THUMBNAIL_FIELD_LABELS: Record<string, string> = {
  layout: "Layout",
                    ...
                    <div className="flex flex-col gap-1 flex-shrink-0">
                      ...
                      {h.thumbnail && (
                        <Button
                          ...
                          onClick={() => setExpandedThumbnail(expandedThumbnail === i ? null : i)}
                        >
                          {expandedThumbnail === i ? (
                            <><ChevronUp className="w-3 h-3 mr-1" /> Hide Thumb</>
                          ) : (
                            <><ChevronDown className="w-3 h-3 mr-1" /> Thumbnail</>
                          )}
                        </Button>
                      )}
                    </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {(Object.keys(THUMBNAIL_FIELD_LABELS) as Array<keyof typeof THUMBNAIL_FIELD_LABELS>).map((field) => (
                          <div key={field} className={`rounded-md bg-muted/40 px-3 py-2 ${
                            field === 'productionNotes' ? 'sm:col-span-2' : ''
                          }`}>
                            <div className="text-xs font-medium text-muted-foreground mb-0.5">
                              {THUMBNAIL_FIELD_LABELS[field]}
                            </div>
                            <div className="text-xs text-foreground leading-relaxed">
                              {h.thumbnail[field as keyof typeof h.thumbnail]}
                            </div>
                          </div>
                        ))}
                      </div>

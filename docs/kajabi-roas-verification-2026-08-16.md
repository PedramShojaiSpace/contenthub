...
## Shopify Paid-Order Evidence Follow-Up

The active Shopify `ORDERS_PAID` subscription is registered to the Content Hub paid-order receiver and was created on 2026-08-11. Its local `attributed_sales` ledger, however, contained only three unattributed rows from 2026-07-05 and no later deliveries. The supplied Shopify signing secret now validates successfully against the live receiver’s HMAC authorization path through a deliberately malformed, non-order local payload; the receiver returns a safe 400 after authorization and before any persistence, Klaviyo, or CAPI work. This confirms the secret is correctly available to the runtime without generating a test order, artificial sale, or Meta Purchase event. Full regression passes at **162 files / 1,556 tests / 2 intentional skips**. The next genuine Shopify paid order remains the required passive production confirmation for a fresh `attributed_sales` record and its corresponding CAPI audit.

## Kajabi Native Tracking — Read-Only Configuration Inspection

Kajabi’s **Integrations & Webhooks** page exposes its native **Facebook Pixel** toggle together with **Pixel ID** and **Access Token** fields. The authenticated read-only page did not expose the stored values in its accessible text, and no toggle, field, save action, checkout configuration, or thank-you configuration was changed. This confirms the native tracking surface is present but does not by itself prove event-level deduplication. The current Content Hub CAPI audit must therefore remain the measurement authority until a controlled, approval-gated real-order comparison is performed.

## KO/Klaviyo Email-to-Revenue Collector Verification

The managed collector job `interconnected-email-performance-daily` is enabled on the UTC schedule `0 15 15 * * *` and has the authoritative task UID `PN4tSqosxNU94dEpREqYAN`. Its first managed execution completed successfully on **2026-08-16T15:20:56Z** with HTTP 200 in 1.626 seconds, without a manual trigger or retry. The response records an isolated completed-day window of **2026-08-02T00:00:00Z** through **2026-08-16T00:00:00Z**, a collection timestamp of **2026-08-16T15:20:56Z**, and **7** message rows from that scheduled collection.

The persisted reporting settings match the managed task UID and timestamp. The KO/Klaviyo snapshot table contains only the `ko_klaviyo` / `klaviyo` path for this collector, with no Kajabi collection in the same record. It currently contains 27 accumulated message rows across collection windows, including 105 recipients and 47 clicks; the scheduler’s first successful run added the recorded seven-row completed-day collection. The handler does call the owner-notification service during its first run, but it does not persist the boolean delivery result. The scheduler response has no notification field and the owner-notification inbox has no read-only retrieval API in this environment. Accordingly, the collector success and data isolation are verified, while **actual notification delivery cannot be independently proven from available records**. No Meta API call was made during this verification.

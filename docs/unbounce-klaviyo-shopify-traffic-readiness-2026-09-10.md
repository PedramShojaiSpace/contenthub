...
Post-deployment verification identified that the first `/cart/update` implementation did **not** persist attributes in the active storefront session: the visitor reached the correct product page with the tracked query string, but a same-session `cart.js` read returned `attributes: {}`. This keeps direct Shopify attribution as an active blocker. The implementation must be corrected and re-verified at the anonymous-cart layer before any paid-order test or ROAS claim.

The initial failure was traced to deployment lag, not to Shopify’s form endpoint. While the custom domain was still returning the prior 302 bridge, the local server already returned the new form. After the production rollout completed, `/r/checkout` returned HTTP 200 with the Shopify `/cart/update` form, and a fresh public CTA click reached the same $67 product page with a newly issued 48-character first-party token. A same-session cart inspection is the final anonymous-cart verification step.

The final clean-session probe passed. Starting from the deployed `/r/checkout` HTML, the test extracted the newly issued token, posted the exact hidden fields to Shopify `/cart/update`, retained the anonymous Shopify cart cookie, and read `cart.js`. Shopify returned `item_count: 0`, the exact token, `ko_klaviyo`, and the complete source/medium/campaign/content attributes. No product was added and no checkout or order was created. The remaining evidence gate is the next genuine or separately approved paid order, which must show the same token in order note attributes and a single first-party paid-order attribution record.

With explicit owner approval, one real email-only LP-3 validation was submitted on September 13, 2026 using a fresh alias of the owner’s existing address. The phone field remained blank and the optional SMS checkbox remained unchecked. The browser immediately redirected from the public Unbounce form to `https://content.theurbanmonk.com/interconnected/thank-you-klaviyo`, where the active 15-minute offer state rendered. No order, payment, SMS enrollment, or ad activation occurred. Downstream first-party, Klaviyo, Meta CAPI, Unbounce conversion, and summary checks follow this browser-level proof.

The downstream controlled-test checks passed for lead delivery: exactly one first-party `ko_klaviyo` row was created; phone was blank; SMS consent was false; Klaviyo sync was true; and Meta CAPI Lead delivery was accepted. Klaviyo read-back confirmed membership in trigger list `Rrx44Q`, email marketing `SUBSCRIBED`, SMS `NEVER_SUBSCRIBED`, and one `Received Email` event tied to live flow `YyFZPu` and Day 0 message `SypiFq`. Unbounce’s all-time page statistic remained at six conversions because the provider deduplicates conversion goals by visitor and the validation reused an existing browser; it is not a raw submission counter.

The validation also exposed a remaining attribution defect. The native server webhook received a queryless `page_url` and therefore stored its fallback campaign values rather than the launch UTMs. LP-3 still contains a legacy custom script that listens for a `klaviyoForms` event even though the active form is native Unbounce. The Content Hub browser bridge is now hardened so a corrected LP script can enrich the existing native lead’s UTMs and browser identifiers while suppressing a second CAPI Lead if the native webhook already sent one. Updating the LP-3 custom script requires authenticated Unbounce editor access.

After the controlled lead completed, the provider-side integration flag was queried again. LP-3 no longer appears when Unbounce is filtered to `integration_errors: true`. The earlier warning therefore cleared through the successful native delivery and is no longer a launch blocker; no integration was removed or remapped.

The owner then added and published the supplied native-form attribution script at Before Body End. Public source verification confirmed the new form listener is live. The first post-publish validation still stored fallback UTMs, and code review isolated the cause in the Content Hub rather than Unbounce: the browser bridge allowlist accepted only the older `/interconnected-lp/` path while the live page is `/interconnected-lp-3/`. The bridge now accepts both exact Interconnected paths. Both the browser bridge and the native server webhook also suppress their own CAPI delivery when the other path has already marked the same-day lead as CAPI-sent, removing the duplicate-race risk. Focused funnel tests pass 29/29 and the bounded-memory production build passes. A single replay of the existing final QA browser event after deployment is sufficient to verify UTM enrichment without creating another Klaviyo profile or Day 0 email.

The replay proved the corrected server behavior on the current project runtime: it returned HTTP 200 with `deduplicated: true`, enriched the existing QA row, and emitted no new CAPI Lead. The custom-domain endpoint still returned the old HTTP 400 validation response after checkpoint `e6b29ae3`, indicating deployment propagation/stale server code rather than a payload or implementation defect. A fresh deployment is required before the public LP script can complete browser-UTM enrichment automatically.

The final cache-busted production rollout succeeded. The public custom-domain bridge accepted the same existing QA replay and returned HTTP 200 with `deduplicated: true`. Aggregate database verification confirmed one matching email-only/no-SMS `ko_klaviyo` row, Klaviyo sync true, CAPI Lead true, exactly one distinct CAPI event ID, and the complete final-validation source/medium/campaign/content values. The replay created no additional Klaviyo profile, Day 0 email, Meta Lead, product, cart item, checkout, order, or payment. Browser-UTM enrichment is therefore closed.

The hourly owner-summary deployment is also verified. Heartbeat run `QkT4p2jMWJGaseUpsmi5FC` completed with HTTP 200 and returned the new path-specific payload: one in-window KO/Klaviyo lead, zero Kajabi leads, and zero unassigned leads. The job remains enabled on its hourly cadence and still emits one summary rather than per-lead notifications.

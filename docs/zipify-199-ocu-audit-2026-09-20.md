...
## Live re-verification — 2026-09-20, 15:44 Central

The authenticated Shopify administrator still shows **One Click Upsell** in the installed-app navigation. A guessed direct Shopify app URL returned Shopify’s standard 404, so the current verification must launch the installed app through that navigation rather than relying on a constructed URL. No setting or buyer-facing configuration changed during this check.

Launching the installed navigation opened Zipify’s current embedded application successfully, including its **Funnels**, **Statistics**, and **Settings** navigation. The initial dashboard surface was still loading at capture time, so the live funnel details require inspection through Zipify’s own Funnels view rather than the Shopify app shell. No setting was changed.

The live **Upsell Funnels** view shows the row named **“Interconnected $67 → …”** with its status control enabled. For the selected last-30-days window it reports **0 views, 0% conversion, $0.00 revenue, and $0.00 revenue per visit**. This is consistent with a published, eligible funnel that has not yet been reached by a qualifying $67 checkout—not evidence that the offer is broken. The detailed trigger and offer configuration are being inspected read-only next.

The current embedded Zipify UI did not expose the row’s detail panel to the automated browser after a read-only row/expand interaction; it remained in the loading state and no configuration was altered. The last direct configuration inspection remains the record that the funnel has exactly one selected $67 product trigger and the $199 product in Post-Purchase Upsell #1, with all other offer slots empty. The active row and zero-view status reconfirm its publication state, but do not by themselves re-display the product IDs.

## Post-launch exposure observation — 2026-09-21

A fresh read-only Zipify funnel-statistics view now shows **1 view**, **0% conversion**, **$0.00 revenue**, and **$0.00 revenue per visit** for the published Interconnected $67 → $199 row. Shopify’s aggregate Admin read shows three paid, non-cancelled `$67` trigger-product orders since the launch start: one was processed before the Zipify publication timestamp and two after it. All three used Shopify Payments. The one post-publication view therefore confirms that the native offer can render, but the second post-publication trigger order did not create a recorded view. This is not a presentation verdict and does not establish an accepted/declined outcome.

The immediate next evidence gate is exposure reliability, not a content replacement: observe at least ten paid `$67` orders occurring after publication and require at least 90% Zipify-view coverage before testing the video-led long-form presentation. No Zipify, Shopify, order, checkout, product, price, email, advertising, or traffic setting changed during this observation.

## Current exposure re-verification — 2026-09-22

The published **Interconnected $67 → $199** funnel’s live Zipify row now reports **4 views, 0% conversion, $0.00 revenue, and $0.00 revenue per visit**. A fresh aggregate Shopify Admin read finds six paid, non-cancelled `$67` trigger orders since launch: one pre-publication and five post-publication. All six used Shopify Payments, and none contains the `$199` `FIT-22-OCUS-199` line item. The current native-screen coverage is therefore **4/5 post-publication orders (80%)**.

Zipify reporting does not expose an order-level join in the available statistics, so it cannot prove exactly which individual buyer saw the one-click screen. It does establish that at least one eligible post-publication order did not produce a recorded view. This makes the immediate issue exposure reliability, not offer-page effectiveness; no presentation change has been made.

The app’s **Split Tests Dashboard** is available and currently has no tests. Once the existing exposure gate reaches ten post-publication orders at at least 90% view coverage, the correct next test is a native Zipify **Same Product** Page B clone carrying the video-led Shopify-page content while retaining the same trigger, $199 product, price, eligibility, and one-click post-purchase position. Do not route the buyer to the ordinary Shopify product page, because that would add checkout friction and invalidate a one-click presentation comparison. Full read-only test plan: `docs/zipify-exposure-verification-and-video-page-test-plan-2026-09-22.md`.

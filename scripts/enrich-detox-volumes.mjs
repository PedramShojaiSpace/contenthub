/**
 * Enrich Detox & Toxicity keyword_targets with DataForSEO search volume, difficulty, and CPC.
 * Targets only the "Detox & Toxicity Authority" campaign (pillar = "heavy metal detox").
 * Run: node scripts/enrich-detox-volumes.mjs
 */
...
async function main() {
  ...
  try {
    // Find the detox campaign
    const [camps] = await conn.execute(
      "SELECT id FROM keyword_campaigns WHERE kc_pillar_keyword = ? LIMIT 1",
      ["heavy metal detox"]
    );
    if (!camps.length) {
      console.error("Detox & Toxicity campaign not found. Run seed-detox-toxicity.mjs first.");
      return;
    }
    const campaignId = camps[0].id;
    console.log(`✓ Found Detox & Toxicity Authority campaign (id=${campaignId})`);
    ...
    console.log("\n✓ Volume enrichment complete for Detox & Toxicity Authority campaign.");
  } finally {
    await conn.end();
  }
}

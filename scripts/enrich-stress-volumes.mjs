/**
 * Enrich Stress & Cortisol keyword_targets with DataForSEO search volume, difficulty, and CPC.
 * Targets only the "Stress & Cortisol Authority" campaign (pillar = "cortisol and stress").
 * Run: node scripts/enrich-stress-volumes.mjs
 */
...
async function main() {
  ...
  try {
    // Find the stress campaign
    const [camps] = await conn.execute(
      "SELECT id FROM keyword_campaigns WHERE kc_pillar_keyword = ? LIMIT 1",
      ["cortisol and stress"]
    );
    if (!camps.length) {
      console.error("Stress & Cortisol campaign not found. Run seed-stress-cortisol.mjs first.");
      return;
    }
    const campaignId = camps[0].id;
    console.log(`✓ Found Stress & Cortisol Authority campaign (id=${campaignId})`);
    ...
    console.log("\n✓ Volume enrichment complete for Stress & Cortisol Authority campaign.");
  } finally {
    await conn.end();
  }
}

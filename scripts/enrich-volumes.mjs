/**
 * Enrich keyword_targets with DataForSEO search volume, difficulty, and CPC.
 * Run: node scripts/enrich-volumes.mjs
 */
...
async function getKeywordVolumes(keywords) {
  // DataForSEO search_volume/live: send all keywords in a SINGLE task with keywords array
  const body = [{
    keywords: keywords,
...
async function main() {
  ...
  try {
    // Get all keywords without volume data
    const [rows] = await conn.execute(
      "SELECT id, kt_keyword FROM keyword_targets WHERE kt_search_volume IS NULL ORDER BY kt_priority ASC"
    );
    ...
    // Batch in groups of 10 (DataForSEO limit per request)
    const BATCH = 10;

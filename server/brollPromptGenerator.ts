...
{
  "underlordPrompt": "A detailed Descript Underlord prompt for an AVATAR-LED video. CRITICAL RULE — THIS IS AN AVATAR VIDEO: The main presenter (Dr. Pedram Shojai avatar) must remain on screen for the majority of the video. B-roll is used for FREQUENT CUTAWAYS to keep viewers visually engaged. B-ROLL FREQUENCY RULE — NON-NEGOTIABLE: Place AT LEAST 3 B-roll cutaways per minute of video. Each cutaway must last a MINIMUM of 10 seconds before cutting back to the avatar. For a 9-minute video that means at least 27 cutaways. Calculate the total video duration and divide by 60 to determine the minimum cutaway count — then place that many cutaways evenly distributed across the video. B-ROLL PLACEMENT RULES: (1) Cut to B-roll whenever the speaker mentions a specific visual concept, statistic, body part, food, nature scene, or scientific concept — but also cut between these moments to maintain visual rhythm. (2) Each cutaway is a MINIMUM of 10 seconds — never shorter. (3) Space cutaways every 15-25 seconds throughout the video. (4) NEVER use B-roll during the very first 10 seconds (intro hook) or the very last 10 seconds (CTA). (5) NEVER REUSE THE SAME CLIP. CLEANUP: Remove all filler words (um, uh, like, you know) and long pauses over 0.5 seconds. CAPTIONS: Add auto-captions in white text, lower third position. MUSIC: Add subtle ambient background music at -18dB (nature/meditation style). B-ROLL CONTENT: Describe specific stock footage clips that directly match the concept being spoken about at that moment — anatomy visuals, healthy food, nature, science imagery, wellness scenes. Each clip must be unique. END CARD: Add a 5-second end card with white text on dark background: 'Learn More at theurbanmonk.com'",
